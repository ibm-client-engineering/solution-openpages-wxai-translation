...
If the steps above were not successful, go back to the console button, click on the three dots next to it, and select reboot. Relogin with the administrator username and password. Then follow the process below: 
    ...
    6. Lastly, go back to the Techzone reservation, under `Published Services`: click on the OpenPages url link. If successful, the login page for OpenPages will populate on a new tab.
        * Username/Password: `OpenPagesAdministrator`
